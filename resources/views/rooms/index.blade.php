<x-app-layout>
    <div class="bg-red-900 pt-8 pb-24 px-4 sm:px-6 lg:px-8">
        <div class="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div>
                <a href="{{ route('rooms.create') }}" class="inline-flex items-center px-6 py-3 bg-white text-red-900 font-bold rounded-xl hover:bg-red-50 transition shadow-lg transform hover:-translate-y-0.5">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-16 pb-12">
        ...
        <div class="bg-white rounded-2xl shadow-lg p-4 mb-8 flex flex-col md:flex-row items-center justify-between gap-4">
            <form method="GET" action="{{ route('rooms.index') }}" class="relative w-full md:w-96">
            ...
            <div class="text-sm text-gray-500">
                Menampilkan <strong>{{ $rooms->count() }}</strong> Ruangan
            </div>
        </div>
                    ...
                    <div class="flex justify-between items-start mb-2">
                        <div>
                            <p class="text-xs font-bold text-red-600 uppercase tracking-wider mb-1">
                                {{ $room->building->name ?? 'Gedung Umum' }}
                            </p>
                            <h3 class="text-xl font-bold text-gray-900 leading-tight">{{ $room->name }}</h3>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-between pt-4 border-t border-gray-100 mt-auto">
                        ...
                        @if(Auth::user()->role === 'admin')
                            <div class="flex space-x-2">
                                <a href="{{ route('rooms.edit', $room->id) }}" class="text-gray-400 hover:text-blue-600 transition">
                        ...
                        @else
                           <a href="{{ route('bookings.create') }}" class="text-sm font-bold text-red-700 hover:underline">
                               Pinjam &rarr;
                           </a>
                        @endif
                    </div>
