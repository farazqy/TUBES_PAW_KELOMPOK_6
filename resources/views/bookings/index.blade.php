<x-app-layout>
    <div class="bg-red-900 pt-8 pb-24 px-4 sm:px-6 lg:px-8">
        <div class="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div>
                <a href="{{ route('bookings.create') }}" class="inline-flex items-center px-6 py-3 bg-white text-red-900 font-bold rounded-xl hover:bg-red-50 transition shadow-lg transform hover:-translate-y-0.5">
            ...
            @if(session('success'))
                <div class="bg-green-50 border-l-4 border-green-500 p-4 m-6 mb-0 rounded-r">
                    <div class="flex">
                        ...
                        <div class="ml-3"><p class="text-sm text-green-700 font-bold">{{ session('success') }}</p></div>
                    </div>
                </div>
            @endif
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-gray-50">
                        ...
                        <tr class="hover:bg-red-50/30 transition duration-150 {{ $booking->status == 'pending' && Auth::user()->role == 'admin' ? 'bg-yellow-50' : '' }}">
                            
                            <td class="px-6 py-5">
                                <div class="flex items-center">
                                    <div class="h-10 w-10 rounded-lg bg-red-100 text-red-600 flex items-center justify-center font-bold text-xs">
                                        {{ substr($booking->room->name, 0, 3) }}
                                    </div>
                                    <div class="ml-4">
                                        <div class="text-sm font-bold text-gray-900">{{ $booking->room->name }}</div>
                                        <div class="text-xs text-gray-500 mt-1 max-w-[200px] truncate" title="{{ $booking->purpose }}">
                                            {{ $booking->purpose }}
                                        </div>
                                    </div>
                                </div>
                            ...
                            <td class="px-6 py-5">
                                <div class="text-sm text-gray-900 font-medium">
                                    {{ \Carbon\Carbon::parse($booking->start_time)->isoFormat('dddd, D MMMM Y') }}
                                </div>
                            ...
                            <td class="px-6 py-5 text-right">
                                ...
                                @if(Auth::user()->role === 'admin' && $booking->status == 'pending')
                                    <div class="flex items-center justify-end space-x-2">
                                        ...
                                        <form action="{{ route('bookings.reject', $booking->id) }}" method="POST" onsubmit="return confirm('Tolak pengajuan ini?');">
                                            @csrf @method('PATCH')
                                ...
                                @else
                                    @if($booking->status == 'approved')
                                        <a href="{{ route('bookings.ticket', $booking->id) }}" target="_blank" 
